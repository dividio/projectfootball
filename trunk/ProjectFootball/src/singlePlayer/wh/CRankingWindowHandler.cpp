...
void CRankingWindowHandler::enter()
{
    ...
    if(lastMatch->getXMatch() > 0) {
        ...
        CPfCompetitionPhases *lastCompetitionPhase = m_game.getIDAOFactory()->getIPfCompetitionPhasesDAO()->findByXCompetitionPhase(lastPhaseRound->getXFkCompetitionPhase_str());
