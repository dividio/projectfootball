...
std::vector<CPfMatches*>* CPfMatchesDAOSQLite::findByXFkPhaseRoundAndXFkSeason(int XFkPhaseRound, int XFkSeason)
{
    ...
    return findByXFkPhaseRoundAndXFkSeason(stream.str(), strXFkSeason.str());
}

std::vector<CPfMatches*>* CPfMatchesDAOSQLite::findByXFkPhaseRoundAndXFkSeason(const std::string &XFkPhaseRound, const std::string &XFkSeason)
{
    std::string sql("SELECT * FROM PF_MATCHES WHERE ");
    sql += "X_FK_PHASE_ROUND='"+XFkPhaseRound+"' AND X_FK_SEASON='"+XFkSeason+"'";
    return loadVector(sql);
}
