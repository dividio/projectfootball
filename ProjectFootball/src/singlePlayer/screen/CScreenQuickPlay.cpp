...
CScreenQuickPlay::CScreenQuickPlay()
    :CScreen()
{
    CLog::getInstance()->debug("CScreenQuickPlay()");
}
...
CScreenQuickPlay* CScreenQuickPlay::getInstance()
{
    static CScreenQuickPlay instance;
    return &instance;
}

void CScreenQuickPlay::enter()
{
    newQuickGame();
    CScreenSimulator::getInstance()->enter();
}

void CScreenQuickPlay::forcedLeave()
{
    CScreenSimulator::getInstance()->forcedLeave();
    CGameEngine::getInstance()->unloadCurrentGame();
    deleteQuickGames();
}

bool CScreenQuickPlay::leave()
{
    if( CScreenSimulator::getInstance()->leave() ){
        CGameEngine::getInstance()->unloadCurrentGame();
        deleteQuickGames();
        return true;
    }else{
...
void CScreenQuickPlay::update()
{
    CScreenSimulator::getInstance()->update();
}

void CScreenQuickPlay::newQuickGame()
{
    ...
    if( user==NULL || user->getXUser()==0 ){
        CLog::getInstance()->exception("[CScreenQuickPlay::newQuickGame] User not defined");
    }
    ...
    if( teamsList->empty() ){
        CLog::getInstance()->exception("[CScreenQuickPlay::newQuickGame] Teams list is empty");
    }
    ...
    if( gamesList->size()!=1){
        CLog::getInstance()->exception("[CScreenQuickPlay::newQuickGame] New game isn't correctly created [gamesList->size():%d]", gamesList->size());
    }
...
void CScreenQuickPlay::deleteQuickGames()
{
