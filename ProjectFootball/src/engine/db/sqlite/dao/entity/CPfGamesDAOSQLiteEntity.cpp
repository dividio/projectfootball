...
bool CPfGamesDAOSQLiteEntity::deleteReg(CPfGames *reg)
{
    ...
    sql = sql+"'"+reg->getXGame()+"'";
    return exec(sql);
}

bool CPfGamesDAOSQLiteEntity::insertReg(CPfGames *reg)
{
    std::string sql("INSERT INTO PF_GAMES (X_FK_USER,S_CONNECTION_STRING,X_GAME,S_DRIVER_NAME,S_GAME_NAME) VALUES (");
    sql = sql+" '"+reg->getXFkUser()+"'";
    sql = sql+",'"+reg->getSConnectionString()+"'";
    sql = sql+",'"+reg->getXGame()+"'";
    sql = sql+",'"+reg->getSDriverName()+"'";
    sql = sql+",'"+reg->getSGameName()+"'";
    sql = sql+")";
    return exec(sql);
}
...
bool CPfGamesDAOSQLiteEntity::updateReg(CPfGames *reg)
{
    ...
    sql = sql+" X_FK_USER='"+reg->getXFkUser()+"'";
    sql = sql+",S_CONNECTION_STRING='"+reg->getSConnectionString()+"'";
    sql = sql+",X_GAME='"+reg->getXGame()+"'";
    sql = sql+",S_DRIVER_NAME='"+reg->getSDriverName()+"'";
    sql = sql+",S_GAME_NAME='"+reg->getSGameName()+"'";
    sql = sql+" WHERE X_GAME='"+reg->getXGame()+"'";
    return exec(sql);
}
