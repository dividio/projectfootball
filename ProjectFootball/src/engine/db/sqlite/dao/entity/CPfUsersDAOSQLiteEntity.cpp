...
bool CPfUsersDAOSQLiteEntity::updateReg(CPfUsers *reg)
{
    ...
    sql = sql+" X_USER='"+reg->getXUser_str()+"'";
    sql = sql+",S_USER='"+reg->getSUser_str()+"'";
    sql = sql+" WHERE X_USER='"+reg->getXUser_str()+"'";
    return exec(sql);
}
    ...
    if( object!=NULL ){
        ...
        for( int i=0; i<nColumns; i++ ){
            if( strcmp(sColumn[i], "X_USER")==0 ){
                destiny->setXUser_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "S_USER")==0 ){
                destiny->setSUser_str(vColumn[i]);
            }
        }
        error = 0; // All OK
    }
