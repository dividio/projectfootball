...
bool CPfUsersDAOSQLiteEntity::deleteReg(CPfUsers *reg)
{
    ...
    sql = sql+"'"+reg->getXUser()+"'";
    return exec(sql);
}

bool CPfUsersDAOSQLiteEntity::insertReg(CPfUsers *reg)
{
    std::string sql("INSERT INTO PF_USERS (X_USER,S_USER) VALUES (");
    sql = sql+" '"+reg->getXUser()+"'";
    sql = sql+",'"+reg->getSUser()+"'";
    sql = sql+")";
    return exec(sql);
}
...
bool CPfUsersDAOSQLiteEntity::updateReg(CPfUsers *reg)
{
    ...
    sql = sql+" X_USER='"+reg->getXUser()+"'";
    sql = sql+",S_USER='"+reg->getSUser()+"'";
    sql = sql+" WHERE X_USER='"+reg->getXUser()+"'";
    return exec(sql);
}
...
CPfUsers* CPfUsersDAOSQLiteEntity::loadRegister(std::string sql)
{
    char *msgError = NULL;
...
std::vector<CPfUsers*> * CPfUsersDAOSQLiteEntity::loadVector(std::string sql)
{
    char *msgError = NULL;
...
bool CPfUsersDAOSQLiteEntity::exec(std::string sql)
{
    char *msgError = NULL;
    bool correct = true;
    ...
    if( object!=NULL ){
        ...
        for( int i=0; i<nColumns; i++ ){
            if( strcmp(sColumn[i], "X_USER")==0 ){
                destiny->setXUser(vColumn[i]);
            }else if( strcmp(sColumn[i], "S_USER")==0 ){
                destiny->setSUser(vColumn[i]);
            }
        }
        error = 0; // All OK
    }
