...
{
    std::string sql("INSERT INTO PF_GOALS (X_FK_TEAM_SCORER,N_MINUTE,X_FK_MATCH,L_OWN_GOAL) VALUES (");
...
int CPfGoalsDAOSQLiteEntity::callbackRegister(void *object, int nColumns, char **vColumn, char **sColumn)
{
    ...
    if( object!=NULL ){
        ...
        for( int i=0; i<nColumns; i++ ){
            if( strcmp(sColumn[i], "X_FK_TEAM_SCORER")==0 ){
                destiny->setXFkTeamScorer(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_GOAL")==0 ){
                destiny->setXGoal(vColumn[i]);
            }else if( strcmp(sColumn[i], "N_MINUTE")==0 ){
                destiny->setNMinute(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_FK_MATCH")==0 ){
                destiny->setXFkMatch(vColumn[i]);
            }else if( strcmp(sColumn[i], "L_OWN_GOAL")==0 ){
                destiny->setLOwnGoal(vColumn[i]);
            }
        }
        error = 0; // All OK
    }
