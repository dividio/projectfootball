...
bool CPfGoalsDAOSQLiteEntity::deleteReg(CPfGoals *reg)
{
    ...
    sql = sql+"'"+reg->getXGoal_str()+"'";
    return exec(sql);
}

bool CPfGoalsDAOSQLiteEntity::insertReg(CPfGoals *reg)
{
    std::string sql("INSERT INTO PF_GOALS (X_FK_TEPL_SCORER,L_OWN_GOAL,X_FK_TEAM_SCORER,X_FK_MATCH,N_MINUTE) VALUES (");
    sql = sql+" '"+reg->getXFkTeplScorer_str()+"'";
    sql = sql+",'"+reg->getLOwnGoal_str()+"'";
    sql = sql+",'"+reg->getXFkTeamScorer_str()+"'";
    sql = sql+",'"+reg->getXFkMatch_str()+"'";
    sql = sql+",'"+reg->getNMinute_str()+"'";
    sql = sql+")";
    return exec(sql);
}
...
bool CPfGoalsDAOSQLiteEntity::updateReg(CPfGoals *reg)
{
    ...
    sql = sql+" X_FK_TEPL_SCORER='"+reg->getXFkTeplScorer_str()+"'";
    sql = sql+",X_GOAL='"+reg->getXGoal_str()+"'";
    sql = sql+",L_OWN_GOAL='"+reg->getLOwnGoal_str()+"'";
    sql = sql+",X_FK_TEAM_SCORER='"+reg->getXFkTeamScorer_str()+"'";
    sql = sql+",X_FK_MATCH='"+reg->getXFkMatch_str()+"'";
    sql = sql+",N_MINUTE='"+reg->getNMinute_str()+"'";
    sql = sql+" WHERE X_GOAL='"+reg->getXGoal_str()+"'";
    return exec(sql);
}
...
int CPfGoalsDAOSQLiteEntity::callbackRegister(void *object, int nColumns, char **vColumn, char **sColumn)
{
    ...
    if( object!=NULL ){
        ...
        for( int i=0; i<nColumns; i++ ){
            if( strcmp(sColumn[i], "X_FK_TEPL_SCORER")==0 ){
                destiny->setXFkTeplScorer_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_GOAL")==0 ){
                destiny->setXGoal_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "L_OWN_GOAL")==0 ){
                destiny->setLOwnGoal_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_FK_TEAM_SCORER")==0 ){
                destiny->setXFkTeamScorer_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_FK_MATCH")==0 ){
                destiny->setXFkMatch_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "N_MINUTE")==0 ){
                destiny->setNMinute_str(vColumn[i]);
            }
        }
        error = 0; // All OK
    }
