...
bool CPfCompetitionPhasesDAOSQLiteEntity::deleteReg(CPfCompetitionPhases *reg)
{
    ...
    sql = sql+"'"+reg->getXCompetitionPhase_str()+"'";
    return exec(sql);
}
...
bool CPfCompetitionPhasesDAOSQLiteEntity::insertReg(CPfCompetitionPhases *reg)
{
    ...
    sql = sql+" '"+reg->getNOrder_str()+"'";
    sql = sql+",'"+reg->getSCompetitionPhase_str()+"'";
    sql = sql+",'"+reg->getXFkCompetition_str()+"'";
    sql = sql+")";
    return exec(sql);
}
...
bool CPfCompetitionPhasesDAOSQLiteEntity::updateReg(CPfCompetitionPhases *reg)
{
    ...
    sql = sql+" N_ORDER='"+reg->getNOrder_str()+"'";
    sql = sql+",X_COMPETITION_PHASE='"+reg->getXCompetitionPhase_str()+"'";
    sql = sql+",S_COMPETITION_PHASE='"+reg->getSCompetitionPhase_str()+"'";
    sql = sql+",X_FK_COMPETITION='"+reg->getXFkCompetition_str()+"'";
    sql = sql+" WHERE X_COMPETITION_PHASE='"+reg->getXCompetitionPhase_str()+"'";
    return exec(sql);
}
    ...
    if( object!=NULL ){
        ...
        for( int i=0; i<nColumns; i++ ){
            if( strcmp(sColumn[i], "N_ORDER")==0 ){
                destiny->setNOrder_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_COMPETITION_PHASE")==0 ){
                destiny->setXCompetitionPhase_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "S_COMPETITION_PHASE")==0 ){
                destiny->setSCompetitionPhase_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_FK_COMPETITION")==0 ){
                destiny->setXFkCompetition_str(vColumn[i]);
            }
        }
        error = 0; // All OK
    }
