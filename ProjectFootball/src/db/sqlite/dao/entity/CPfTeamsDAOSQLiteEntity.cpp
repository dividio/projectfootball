...
{
    if( m_database==NULL ){
        CLog::getInstance()->exception("[CPfTeamsDAOSQLiteEntity::loadRegister] No database connection.");
    }
...
{
    if( m_database==NULL ){
        CLog::getInstance()->exception("[CPfTeamsDAOSQLiteEntity::loadVector] No database connection.");
    }
...
{
    if( m_database==NULL ){
        CLog::getInstance()->exception("[CPfTeamsDAOSQLiteEntity::exec] No database connection.");
    }
    ...
    if( object!=NULL ){
        ...
        for( int i=0; i<nColumns; i++ ){
            if( strcmp(sColumn[i], "X_TEAM")==0 ){
                destiny->setXTeam_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "S_TEAM")==0 ){
                destiny->setSTeam_str(vColumn[i]);
            }
        }
        error = 0; // All OK
    }
