...
{
    std::string sql("INSERT INTO PF_MATCHES (D_MATCH,L_PLAYED,X_FK_TEAM_HOME,X_FK_TEAM_AWAY) VALUES (");
    sql = sql+" '"+reg->getDMatch_str()+"'";
    sql = sql+",'"+reg->getLPlayed_str()+"'";
...
bool CPfMatchesDAOSQLiteEntity::updateReg(CPfMatches *reg)
{
    ...
    sql = sql+" D_MATCH='"+reg->getDMatch_str()+"'";
    sql = sql+",L_PLAYED='"+reg->getLPlayed_str()+"'";
...
int CPfMatchesDAOSQLiteEntity::callbackRegister(void *object, int nColumns, char **vColumn, char **sColumn)
{
    ...
    if( object!=NULL ){
        ...
        for( int i=0; i<nColumns; i++ ){
            if( strcmp(sColumn[i], "D_MATCH")==0 ){
                destiny->setDMatch_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "L_PLAYED")==0 ){
                destiny->setLPlayed_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_FK_TEAM_HOME")==0 ){
                destiny->setXFkTeamHome_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_MATCH")==0 ){
                destiny->setXMatch_str(vColumn[i]);
            }else if( strcmp(sColumn[i], "X_FK_TEAM_AWAY")==0 ){
                destiny->setXFkTeamAway_str(vColumn[i]);
            }
        }
        error = 0; // All OK
    }
